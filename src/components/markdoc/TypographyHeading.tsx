...
export function TypographyHeading({ level, children }: TypographyHeadingProps) {
    ...
    const className = level === 1 ? 'text-3xl font-semibold!' : level === 2 ? 'text-2xl font-semibold!' : 'text-xl font-medium!'
    ...
    return (
        <div id={headingId} className="group relative">
            <Typography as={`h${level}` as keyof React.JSX.IntrinsicElements} variant="display" size={size} className={cn(className, "mb-4")}>
                {children}
