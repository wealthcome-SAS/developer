  ...
  [
    ...
    [
      ...
      [
        "Company",
        [
          ["List all companies", "financial-provider-specification/company-list"],
          ["Get company detailed information", "financial-provider-specification/company-detail"],
        ],
      ],
      [
        "Manager",
        [
          ["List all managers", "financial-provider-specification/manager-list"],
          ["Get manager detailed information", "financial-provider-specification/manager-detail"],
        ],
      ],
      [
        "Contract",
        [
          ["List all contracts", "financial-provider-specification/contract-list"],
          ["Get contract detailed information", "financial-provider-specification/contract-detail"],
        ],
      ],
      [
        "Asset",
        [
          ...
          ["Get asset detailed information", "financial-provider-specification/asset-detail"],
          ["List all transactions related to an asset", "financial-provider-specification/asset-transactions"],
          ["The valuation in time of an asset", "financial-provider-specification/asset-valuation-history"],
          ["The current valuation of an asset", "financial-provider-specification/asset-valuation-current"],
          ["List all investments currently held in an asset", "financial-provider-specification/asset-investments"],
          ["The history of investments in an asset", "financial-provider-specification/asset-investments-history"],
        ],
      ],
      [
        "Schemas",
        [
          ["cursor_paginated", "financial-provider-specification/schemas/cursor_paginated"],
          ...
          ["transaction", "financial-provider-specification/schemas/transaction"],
    ...
    [
      ...
      [
        ...
        [
          ...
          ["Retrieve list companies statistics", "aggregated-api/company-statistics"],
      ...
      [
        "Reference",
        [
          ["Customer references list statistics", "aggregated-api/customer-references-statistics"],
      ...
      [
        ...
        [
          ...
          ["Retreive the parent company informations", "aggregated-api/parent-company-informations"],
        ],
      ],
      [
        "Schemas",
  ...
  return (
    <>
      {docsTree.map((item, idx) => (
        <Tree key={idx} item={item} currentSection={currentSection} parentPath="" />
      ))}
    </>
  )
}

function Tree({ item, currentSection, parentPath }: { item: DocLeaf | DocNode, currentSection: string | null, parentPath: string }) {
  const name = item[0]
  ...
  if (typeof item[1] === 'string') {
    ...
    const currentSlug = location.pathname.startsWith('/docs/') ? location.pathname.replace('/docs/', '') : null
    const currentHash = location.hash ? location.hash.substring(1) : null
    const effectiveCurrentSlug = currentSlug && currentHash ? `${currentSlug}/${currentHash}` : currentSlug
  ...
  return (
    <SidebarMenuItem>
        <Collapsible key={`${fullPath}-${!!currentSection?.startsWith(fullPath)}`} className="group/collapsible [&[data-state=open]>button>svg:first-child]:rotate-90" defaultOpen={currentSection?.startsWith(fullPath)}>
        ...
        <CollapsibleContent>
          <SidebarMenuSub>
            {children.map((sub, i) => (
              ...
              <Tree key={i} item={sub as any} currentSection={currentSection} parentPath={fullPath} />
            ))}
          </SidebarMenuSub>
        </CollapsibleContent>
      </Collapsible>
...
export function AppSidebar({ ...props }: React.ComponentProps<typeof Sidebar>) {
  ...
  const getSectionForSlug = (slug: string) => {
    ...
    if (effectiveSlug === 'authentication/introduction') return 'Authentication'

    if (effectiveSlug === 'financial-provider-specification/introduction') return 'Financial Provider Specification'
    if (effectiveSlug.startsWith('financial-provider-specification/company')) return 'Financial Provider Specification/Company'
    if (effectiveSlug.startsWith('financial-provider-specification/manager')) return 'Financial Provider Specification/Manager'
    if (effectiveSlug.startsWith('financial-provider-specification/contract')) return 'Financial Provider Specification/Contract'
    if (effectiveSlug.startsWith('financial-provider-specification/asset')) return 'Financial Provider Specification/Asset'
    if (effectiveSlug.startsWith('financial-provider-specification/schemas')) return 'Financial Provider Specification/Schemas'

    if (effectiveSlug === 'aggregated-api/introduction') return 'Aggregated API'
    if (effectiveSlug.startsWith('aggregated-api/company')) return 'Aggregated API/Company'
    if (effectiveSlug.startsWith('aggregated-api/customer')) return 'Aggregated API/Customer'
    if (effectiveSlug.startsWith('aggregated-api/team')) return 'Aggregated API/Team'
    if (effectiveSlug.startsWith('aggregated-api/entity') || effectiveSlug.startsWith('aggregated-api/entities')) return 'Aggregated API/Entity'
    if (effectiveSlug.startsWith('aggregated-api/reference')) return 'Aggregated API/Reference'
    if (effectiveSlug.startsWith('aggregated-api/manager')) return 'Aggregated API/Manager'
    if (effectiveSlug.startsWith('aggregated-api/asset')) return 'Aggregated API/Assets'
    if (effectiveSlug.startsWith('aggregated-api/parent-company')) return 'Aggregated API/Parent Company'
    if (effectiveSlug.startsWith('aggregated-api/schemas')) return 'Aggregated API/Schemas'

    return null
  }

  const currentSection = effectiveCurrentSlug ? getSectionForSlug(effectiveCurrentSlug) : null
