...
export const HeroSection: React.FC = () => {
	...
	useEffect(() => {
		...
		const dotInterval = setInterval(() => {
			const randomWireIndex: number = Math.floor(
				Math.random() * inputWires.length,
			);
			const newDot: Dot = {
			...
			setWires((prev: InputWire[]) =>
				prev.map((wire: InputWire, idx: number) =>
					idx === randomWireIndex
	...
	useEffect(() => {
		const animate = () => {
			...
			setOutputWires((prev: OutputWire[]) =>
				prev.map((wire: OutputWire) => ({
					...wire,
					dots: wire.dots
		...
		const transferInterval = setInterval(() => {
			setWires((prev: InputWire[]) => {
				...
				if (completedDots.length > 0) {
					const randomOutputWire: number = Math.floor(
						Math.random() * outputWires.length,
					);
					setOutputWires((prevOut: OutputWire[]) =>
						prevOut.map((wire: OutputWire, idx: number) =>
	...
	const getWavePath = (
		...
		for (let i = 0; i <= steps; i++) {
			...
			const waveOffset: number =
				Math.sin(x * wire.frequency + animatedPhase) * wire.amplitude;

			const straightFactor: number = Math.pow(progress, 2);
			const y: number =
				baseY +
				waveOffset * (1 - straightFactor) +
				(centerY - baseY) * straightFactor;
	...
	const getDotPosition = (
		...
		const waveOffset: number =
			Math.sin(currentX * inputWire.frequency + animatedPhase) *
			inputWire.amplitude;

		const straightFactor: number = progress ** 2;
		const y: number =
			baseY +
			waveOffset * (1 - straightFactor) +
			(centerY - baseY) * straightFactor;
					...
					return (
						<g key={`wire-input-${wire.id}`}>
							...
							{wire.dots.map((dot: Dot) => {
								const pos: { x: number; y: number } = getDotPosition(
									wire,
									dot.progress,
									width,
									height,
					...
					<div className="p-6 aspect-square rounded-3xl bg-white backdrop-blur-sm border-4 border-slate-300">
						...
						>
							<rect
								...
								height="45.4409"
								rx="6.01904"
								transform="rotate(-19.3198 5 3.98242)"
								fill="#04182b"
							/>
							<rect
								x="25.6152"
								...
								height="45.4409"
								rx="6.01904"
								transform="rotate(-19.3198 25.6152 4.11719)"
								fill="#04182b"
							/>
							<path
								d="M60 8.25899C60 10.7015 53.4228 22 53.4228 22C53.4228 22 47 10.7015 47 8.25899C47 4.80225 49.9101 2 53.5 2C57.0899 2 60 4.80225 60 8.25899Z"
								fill="#04182b"
							/>
						</svg>
					</div>
